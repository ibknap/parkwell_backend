...
class ParkAdmin(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    company_admin = models.ForeignKey(Administrator, on_delete=models.CASCADE)
    photo = models.ImageField(_("Park Admin's photo(optional)"), upload_to='administrator_photos/', default="../static/images/avatar.png", blank=True, null=True)
    mobile_number = models.CharField(_("Park Admin's mobile number"), validators=[mobile_num_regex], max_length=20)
    # timestamp
    created_on = models.DateTimeField(_("Created on"), auto_now_add=True)
    
    class Meta:
        verbose_name = _("ParkAdmin")
        verbose_name_plural = _("ParkAdmins")

    def __str__(self):
        return str(self.user)

    def get_absolute_url(self):
        return reverse("ParkAdmin_detail", kwargs={"pk": self.pk})